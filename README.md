...
>
> `build_and_install.sh` 用你的 Apple Development 证书重新封签安装好的包（词库是在
> Xcode 签名之后才注入的，必须重新封一次）。**不要用 ad-hoc 签名** —— ad-hoc 没有
> team identifier，TCC 只能把授权钉在这次构建的 cdhash 上，于是每装一次，麦克风和
> 辅助功能的授权就失效一次：系统设置里还打着勾，`AXIsProcessTrusted()` 已经返回
> false。签名步骤**不能**加 sudo（私钥在你的登录钥匙串里，root 读不到，会报
> `errSecInternalComponent`），脚本因此在安装**之前**就签好。
>
> 如果某个授权曾经在 ad-hoc 时期给过，它那条记录里存的仍是旧的 code requirement，
> 在系统设置里关掉再打开**没有用**（只改授权值，不改 requirement）。删掉整条记录再重给：
>
> ```bash
> tccutil reset Accessibility com.marmotim.inputmethod.MarmotIM
> ```
...
> Command 不会有任何反应，这是刻意的。左 Command 与所有 ⌘ 快捷键都照常工作 —— 按住期间
> 只要敲了别的键，这次听写就会作废并把快捷键放行。
>
> 想在别的输入源下也能听写，打开「设置 → 转写 → 不是当前输入源时也能听写」（默认关）。
> 它需要**辅助功能**授权，且文字会插进系统认定的当前焦点 —— 代价与注意事项见
> [语音转写使用指南 §4](docs/transcribe.md)。
...
- **转写** - 语音转写开关、是否在别的输入源下也能听写、服务状态、语言、热词；
  高级项内可改地址端口、模型变体等
- **导入导出** - 数据库备份与恢复
- **关于** - 版本信息与开发初衷
