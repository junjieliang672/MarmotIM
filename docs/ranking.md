...
1. **层级系统** - 基于匹配类型（完全/前缀）和编码类型（五笔/拼音/英文）的绝对优先级
...
| 层级 | 匹配类型 | 编码类型 | 加成 | 可覆盖 |
|------|----------|----------|------|--------|
| 1 | 完全匹配 | 五笔（3-4字符）/ 英文 | +100B | ✅ 可以 |
| 2 | 完全匹配 | 拼音 | +10B | ✅ 可以 |
| 3 | 前缀匹配 | 五笔 | +1B | ✅ 可以 |
| 4 | 前缀匹配 | 拼音 | 0 | ✅ 可以 |

**注意**：
- 1-2 字符的五笔完全匹配属于第 0 层（保护层级），而非第 1 层。
- 英文只有完全匹配，不支持前缀匹配。

**设计理由**：3-4 字符的五笔编码（三级简码、全码）可以被覆盖，因为用户可能确实偏好其他词。但 1-2 级简码受到保护。英文完全匹配与五笔完全匹配同级，方便用户快速输入英文单词。
...
**设计理由**：长编码通常是拼音或英文单词。完全匹配应排在前缀匹配之上。

### 英文匹配

英文单词匹配具有以下特点：

1. **仅完全匹配**：只有当输入完全匹配英文单词时才会出现在候选列表中，不支持前缀匹配
2. **第 1 层优先级**：与五笔完全匹配（3-4字符）同级，高于拼音完全匹配
3. **大小写敏感**：优先精确匹配大小写（如输入 "The" 优先匹配 "The"），若无精确匹配则回退到小写匹配
...
|------|----------|------|
| `the` | "the" | 第 1 层 |
| `The` | "The"（优先）或 "the"（回退） | 第 1 层 |
| `hello` | "hello" | 第 1 层 |
| `hel` | 无英文匹配（不支持前缀） | - |

**词库来源**：英文词库存储在 `en_table.txt`，包含约 22,000 个常用英文单词。
...
- `MarmotIM/Dictionary/EnglishWordIndex.swift` - 英文词库索引
